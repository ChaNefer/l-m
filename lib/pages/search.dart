...
class _SearchState extends State<Search> with AutomaticKeepAliveClientMixin {
  ...
  Widget build(BuildContext context) {
    super.build(context); // Remember to call super.build.
    return Scaffold(
      appBar: AppBar(
                          ...
                          builder: (_) => StreamBuilder(
                            ...
                            builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
                              if (snapshot.hasError) {
                                print(
                                    "Wystąpił błąd podczas pobierania danych: ${snapshot.error}");
                                return SizedBox();
                              }
                              if (snapshot.hasData) {
                                var snap = snapshot.data;
                                List docs = snap!.docs;
                                print(snapshot.data!.docs.toString()); // Drukowanie dokumentów w snapshocie
                                if (docs.isEmpty) {
                                  print(
                                      "Brak istniejących rozmów dla użytkownika ${doc.id}");
                                  return Conversation(
                                    userId: doc.id,
                                    chatId: 'newChat',
                                  );
                                } else {
                                  print("Znaleziono istniejące rozmowy dla użytkownika ${doc.id}");
                                  print("Pobrane chatId: ${docs[0].get('chatId')}"); // Drukowanie pobranego chatId
                                  return Conversation(
                                    userId: doc.id,
                                    chatId: docs[0].get('chatId').toString(), // Pobieranie chatId
                                  );
                                }
                              }
                              return SizedBox();
